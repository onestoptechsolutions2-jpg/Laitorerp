...
public class IndexModel : AbpPageModel
{
    ...
    // Takes only the module name and flips its CURRENT server-side state, rather than trusting a
    // client-supplied "enabled" value. A round-tripped hidden bool field turned out to be fragile
    // in production - logs showed the posted value literally as the string "value" instead of
    // "True"/"False" on every click (confirmed via reflection against ASP.NET Core's own
    // NonPropertyAttemptedValueIsInvalidAccessor: the failed message format's {0} IS the raw
    // posted value, and the compiled Razor for the old hidden field was verified correct - so
    // something client-side, e.g. a form-filling browser extension, was clobbering it before
    // submit). Failed bool binding silently defaults to false, which meant "Enable" on an
    // already-disabled module was a no-op every time. Deriving the target state server-side makes
    // this immune to whatever mutates the client-side form.
    public async Task<IActionResult> OnPostToggleAsync(string name)
    {
        try
        {
            ...
            await _featureManager.SetAsync(name, currentlyEnabled ? "false" : "true", HostProviderName, null);
        }
